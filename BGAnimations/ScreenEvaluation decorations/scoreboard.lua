...
local function scoreitem(pn,index,scoreindex,drawindex)
	...
	local t = Def.ActorFrame {
		...
		--The main quad
		Def.Quad{
			InitCommand=cmd(xy,framex,framey+(drawindex*spacing)-4;zoomto,frameWidth,frameHeight;halign,0;valign,0;diffuse,color("#333333");diffusealpha,1;diffuserightedge,color("#33333333"));
		...
		Def.Quad{
			InitCommand=cmd(xy,framex,framey+(drawindex*spacing)-4;zoomto,frameWidth,frameHeight;halign,0;valign,0;diffuse,color("#66ccff");diffusealpha,0.2;diffuserightedge,color("#33333300"));
			BeginCommand=function(self)
		...
		LoadFont("Common normal")..{
			Name="judge";
			InitCommand=cmd(xy,framex+10,framey+23+(drawindex*spacing);zoom,0.45;halign,0;maxwidth,(frameWidth-15)/0.35);
			BeginCommand=function(self)
				if #hstable >= 1 and index>= 1 then
		...
		LoadFont("Common normal")..{
			Name="date";
			InitCommand=cmd(xy,framex+10,framey+23+(drawindex*spacing);zoom,0.45;halign,0);
			BeginCommand=function(self)
				if #hstable >= 1 and index>= 1 then
...
t[#t+1] = LoadFont("Common normal")..{
	InitCommand=cmd(xy,framex,framey+(drawindex*spacing);zoom,0.45;halign,0);
	BeginCommand=function(self)
