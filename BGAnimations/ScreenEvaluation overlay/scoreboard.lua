...
local lines_2 = 4 -- number of scores to display
local framex = 18
local framey = 155
local frameWidth = capWideScale(get43size(260),260)
local frameHeight = 37
local spacing = 40.0
...
local function scoreitem(pn,index,scoreindex,drawindex)
	...
	local t = Def.ActorFrame {
		...
		--The main quad
		Def.Quad{
			InitCommand=cmd(xy,framex,framey+(drawindex*spacing)-4;zoomto,frameWidth,frameHeight;halign,0;valign,0;diffuse,color("#333333");diffusealpha,1;diffuserightedge,color("#33333311"););
		...
		Def.Quad{
			InitCommand=cmd(xy,framex,framey+(drawindex*spacing)-4;zoomto,frameWidth,frameHeight;halign,0;valign,0;diffuse,color("#ffff30");diffusealpha,0.25;diffuserightedge,color("#33333300"));
			BeginCommand=function(self)
		...
		Def.Quad{
			Name="mouseOver";
			InitCommand=cmd(xy,framex,framey+(drawindex*spacing)-4;zoomto,frameWidth,frameHeight;halign,0;valign,0;diffuse,getMainColor(1);diffusealpha,0.2;);
			BeginCommand=function(self)
				self:visible(false);
			end;
		};
		...
		--ClearType lamps
		Def.Quad{
			InitCommand=cmd(xy,framex,framey+(drawindex*spacing)-4;zoomto,8,frameHeight;halign,0;valign,0;diffuse,getClearTypeFromScore(pn,hstable[index],2));
			BeginCommand=function(self)
				self:visible(GAMESTATE:IsHumanPlayer(pn));
			end;
		};
		...
		Def.Quad{
			InitCommand=cmd(xy,framex,framey+(drawindex*spacing)-4;zoomto,8,frameHeight;halign,0;valign,0;diffusealpha,0.3;diffuse,getClearTypeFromScore(pn,hstable[index],2));
			BeginCommand=function(self)
				self:visible(GAMESTATE:IsHumanPlayer(pn));
				self:diffuseramp()
		...
		LoadFont("Common normal")..{
			InitCommand=cmd(xy,framex-8,framey+12+(drawindex*spacing);zoom,0.5;strokecolor,0.5,0.5,0,0.4;shadowlength,1);
			BeginCommand=function(self)
				if #hstable >= 1 then
					self:settext(index)
					if equals then
		...
		LoadFont("Common normal")..{
			Name="grade";
			InitCommand=cmd(xy,framex+10,framey+12+(drawindex*spacing);zoom,0.45;halign,0;maxwidth,(frameWidth-15)/0.35);
			BeginCommand=function(self)
		...
		LoadFont("Common normal")..{
			Name="option";
			InitCommand=cmd(xy,framex+10,framey+12+(drawindex*spacing);zoom,0.4;halign,0;maxwidth,(frameWidth-20)/0.4);
			BeginCommand=function(self)
		...
		--cleartype text
		LoadFont("Common normal")..{
			InitCommand=cmd(xy,framex+10,framey+2+(drawindex*spacing);zoom,0.4;halign,0;maxwidth,(frameWidth-15)/0.35);
			BeginCommand=function(self)
		...
		LoadFont("Common normal")..{
			Name="judge";
			InitCommand=cmd(xy,framex+10,framey+24+(drawindex*spacing);zoom,0.45;halign,0;maxwidth,(frameWidth-15)/0.35);
			BeginCommand=function(self)
				if #hstable >= 1 and index>= 1 then
		...
		LoadFont("Common normal")..{
			Name="date";
			InitCommand=cmd(xy,framex+10,framey+24+(drawindex*spacing);zoom,0.45;halign,0);
			BeginCommand=function(self)
				if #hstable >= 1 and index>= 1 then
...
--Text that sits below the scoreboard with some info
t[#t+1] = LoadFont("Common normal")..{
	InitCommand=cmd(xy,framex,framey+(drawindex*spacing);zoom,0.45;halign,0;strokecolor,0.5,0.5,0,0.4;shadowlength,1;zoom,0.50);
	BeginCommand=function(self)
		if scoreindex ~= 0 then
			self:settextf("Rank %d/%d (Rate: %s)",scoreindex,(#hstable),getRate(hstable[1]))
		else
			self:settextf("Out of rank (Max Scores: %d)",lines_2 or 0)
		end;
	end;
};
...
local function Update(self)
	...
	for i=0,drawindex-1 do
		if isOver(self:GetChild("scoreItem"..tostring(i)):GetChild("mouseOver")) then
			self:GetChild("scoreItem"..tostring(i)):GetChild("grade"):visible(false)
			self:GetChild("scoreItem"..tostring(i)):GetChild("judge"):visible(false)
			self:GetChild("scoreItem"..tostring(i)):GetChild("date"):visible(true)
			self:GetChild("scoreItem"..tostring(i)):GetChild("option"):visible(true)
		else
